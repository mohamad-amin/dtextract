...
class DT:
    ...
    def toDotGraph(self, hasHeader=False, columnNames=None):
        dotBook = 'digraph g {' + '\n'
        tabChar = '    '
        dotBook += tabChar + 'forcelabels=true;' + '\n'
        if hasHeader:
            root_label = self._getNamedLabel(self.root, columnNames)
        else:
            root_label = self.root.get_label()
        dotBook += DT._toDotLabel(str(0), root_label)
        dotBook += DT._toDotGraphInternal(self.root, 0, hasHeader, columnNames)
        dotBook += '}'
        return dotBook

    @staticmethod
    def _toDotGraphInternal(node, num, hasHeader=False, columnNames=None):
        if isinstance(node, DTNode):
            ...
            if hasHeader:
                if hasattr(node.left, 'branch'):
                    left_label = DT._getNamedLabel(node.left, columnNames)
                else:
                    left_label = node.left.get_label()
                if hasattr(node.right, 'branch'):
                    right_label = DT._getNamedLabel(node.right, columnNames)
                else:
                    right_label = node.right.get_label()
            else:
                left_label = node.left.get_label()
                right_label = node.right.get_label()

            dotBook = DT._toDotLabel(str(num + 1), left_label)
            dotBook += DT._toDotLabel(str(num + leftNodes), right_label)

            dotBook += DT._dotTo(str(num), str(num+1))
            dotBook += DT._dotTo(str(num), str(num+leftNodes))

            dotBook += DT._toDotGraphInternal(node.left, num+1, hasHeader, columnNames)
            dotBook += DT._toDotGraphInternal(node.right, num+leftNodes, hasHeader, columnNames)
    ...
    @staticmethod
    def _getNamedLabel(node, columnNames):
        return str(columnNames[node.branch.ind]) + ' <= ' + str(node.branch.thresh)
