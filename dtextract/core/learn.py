...
#
# parameters/returns:
#  learnDTNode : (XS -> YS) * (C * int -> [X]) * C ->1
#                 (({eval : X -> bool} * C * C) | None) * float * {eval : X -> Y} * float
#  func        : XS -> YS (original predictor)
#  dist        : D (distribution of data)
#  initCons    : C (initial constraints)
#  params      : ParamsLearn
#  return      : DT
def learnDT(learnDTNode, func, dist, initCons, params):
    ...
    log('Putting gain: ' + str(gain), INFO)
    worklist.put_nowait((-gain, index, dtInternalData, dtLeafData, depth))
    score = dtLeafScore
    size = 1
    ...
    while True:
        ...
        if worklist.empty():
            break
        (minusGain, index, dtInternalData, dtLeafData, depth) = worklist.get_nowait()
        gain = -minusGain
        ...
        if dtInternalData is None:
            ...
            worklist.put_nowait((2.0, index, None, dtLeafData, depth))  # todo: why 2.0 ?
            if gain < -1.5:
        ...
        worklist.put_nowait((-gainLeft, 2*index+1, dtInternalDataLeft, dtLeafDataLeft, depth+1))
        worklist.put_nowait((-gainRight, 2*index+2, dtInternalDataRight, dtLeafDataRight, depth+1))
        ...
        if params.minGain is not None and gain < params.minGain:
            log('Gain too small, ending!', INFO)
            break
        if params.tgtScore is not None and score >= params.tgtScore:
            log('Achieved target score, ending!', INFO)
            break
        if params.maxSize is not None and size >= params.maxSize:
