...
def readCsv(path, hasHeader, dataTypes):
    ...
    if hasHeader:
        column_names = pd.read_csv(path, nrows=1).columns[usecols]
    else:
        column_names = dummies

    log('Columns: ' + str(len(df.columns)), INFO)
    log('Column names:\n' + ''.join(
        (str(i) + ': ' + str(col) + '\n' for (i, col) in zip(range(len(df.columns)), column_names))), INFO)

    return df, res, resMap, dummies, column_names
